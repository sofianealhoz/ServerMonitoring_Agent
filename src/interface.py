...
def display_ram_stats():
    ...
    return render_template('ram_stats.html', ram_stats=ram_stats) 


@app.route('/hdd/stats')
def display_hdd_data():
    # Obtenez les données du disque dur
    hdd_info = get_hdd_info()
    
    # Préparez les données pour être renvoyées au format JSON
    hdd_data = {
        'total': hdd_info.total,
        'used': hdd_info.used,
        'free': hdd_info.free,
        'percent': hdd_info.percent
    }
    
    # Renvoyez les données au format JSON
    json_data = jsonify(hdd_data)
    
    # Renvoyez le template avec les données JSON
    return render_template('hdd_stats.html', hdd_data=hdd_data, json_data=json_data)
